...
const RESET_PHOTOS = [
  ...
  'Roof before Layout',
  'Roof w/ Layout ONLY',
  'Rough Photos (Racking w/ Grounding and wire Management)',
  'All Grounding (Close Up)',
  'Footing Closeup',
  'Fire setbacks',
  'Panels Installed',
  'Wire Management Photos',
  'System Operational (Screenshot)',
  'If (DC) Show Voltage Meter with Strings',
  'Critter Guard (Installed)',
];

export default function ResetPhotoTicket({ ticketId, onClose }: ResetPhotoTicketProps) {
  ...
  const loadPhotoStatus = async () => {
    try {
      ...
      setPhotos(RESET_PHOTOS.map((label, idx) => ({
        id: `reset_${idx}`,
        label,
        isChecked: checkedPhotos.has(`reset_${idx}`),
        photos: photoUrls[`reset_${idx}`] || [],
      })));
    } catch (err) {
  ...
  const handleDeletePhoto = async (photoId: string, photoUrl: string) => {
    try {
      const fileName = photoUrl.split('/reset-photos/')[1];
      if (fileName) {
        await supabase.storage.from('reset-photos').remove([fileName]);
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">Reset Photo Ticket</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Reset Photos</h3>
