/*
  # Create Labor Cost Calculation Function

  1. New Functions
    - `calculate_customer_labor_costs(customer_id)` - Calculates all labor costs for a customer
      - Battery pay from completed installation tickets
      - Hourly pay from time clock entries
      - Per watt pay from completed installation tickets
      - Returns detailed breakdown

  2. Calculation Logic
    - Battery Pay: Uses battery_quantity * installer's battery_pay_rates
    - Hourly Pay: Sum of (clock_out - clock_in) hours * hourly_rate for all time entries
    - Per Watt Pay: system_size_kw * 1000 * installer's ppw_rate
    - Aggregates by worker for detailed breakdown

  3. Notes
    - Only counts completed/closed tickets
    - Only counts clocked out time entries (completed work)
    - Handles multiple workers per customer
*/

-- Function to calculate labor costs for a customer
...
BEGIN
  ...
  WITH hourly_costs AS (
    SELECT 
      ...
      (EXTRACT(EPOCH FROM (tc.clock_out - tc.clock_in)) / 3600) * COALESCE(u.hourly_rate, 0) as hourly_pay
    FROM time_clock tc
    INNER JOIN app_users u ON tc.user_id = u.id
    WHERE tc.customer_id = p_customer_id
  ...
  WITH ppw_costs AS (
    SELECT 
      ...
      u.ppw_rate,
      c.system_size_kw,
      (c.system_size_kw * 1000) * COALESCE(u.ppw_rate, 0) as ppw_pay
    FROM scheduling s
    INNER JOIN customers c ON s.customer_id = c.id
    INNER JOIN app_users u ON s.pv_installer_id = u.id
    WHERE s.customer_id = p_customer_id
